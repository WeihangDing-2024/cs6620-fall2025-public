...
def parse_log_content(log_content):
    ...
    for row in reader:
        ...
        try:
            long_start = float(row[col_long_start]
                               ) if col_long_start is not None else None
            long_end = float(row[col_long_end]
                             ) if col_long_end is not None else None
            short_start = float(
                row[col_short_start]) if col_short_start is not None and row[col_short_start] else None
            short_end = float(
                row[col_short_end]) if col_short_end is not None and row[col_short_end] else None
...
def hello():
    ...
    <p><strong>Build Date:</strong> {datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
...
def load_log_from_path():
    ...
    try:
        ...
        app.logger.error(
            f"Error loading or parsing log file from path '{log_file_path}': {e}")
    ...
    try:
        ...
        for row in reader:
            ...
            if all(col in row for col in ['recordErrorID', 'recordFile', 'exampleExample', 'recordTime']):
                ...
                try:
                    record_time = time_to_seconds(
                        row['recordTime']) if row['recordTime'].strip() else 0.0
                    record_file = row['recordFile'].strip()

                    # Debug logging
                    app.logger.info(
                        f"Processing CSV row: ID={row['recordErrorID']}, File='{record_file}', Time={record_time}")

                    # Skip rows with empty filenames
                    if not record_file:
                        app.logger.warning(
                            f"Skipping row {row['recordErrorID']} - empty filename")
                        continue
...
def save_label():
    ...
    try:
        ...
        with open(output_file, 'a', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['record_id', 'audio_file', 'error_phrase',
                          'start_time', 'end_time', 'duration', 'labeled_at']
